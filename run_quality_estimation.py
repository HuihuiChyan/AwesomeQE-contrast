...
def evaluate(args, model, tokenizer, valid_dataloader, accelerator, global_step, stop_time):
	...
	all_word_labels = []
	if args.do_contrast:
		all_second_sent_outputs = []
	for batch in valid_dataloader:
		
		with torch.no_grad():
			if args.do_contrast:
				batch["do_contrast"] = True
				sent_outputs, second_sent_outputs = model(**batch)
			else:
		...
		if args.do_contrast:
			all_second_sent_outputs.extend(accelerator.gather(second_sent_outputs).detach().cpu().tolist())
	...
	if args.do_contrast:
		second_sent_result = pearson_and_spearman(all_second_sent_outputs, all_sent_labels)
		if args.only_contrast:
			result['pearson'] = second_sent_result['pearson']
			result['spearmanr'] = second_sent_result['spearmanr']	
		else:
			result['contrast_pearson'] = second_sent_result['pearson']
			result['contrast_spearmanr'] = second_sent_result['spearmanr']		
		...
		for batch in train_dataloader:
			...
			if args.do_contrast:
				...
				for i in range(len(batch["contrast_input_ids"][0])):
					if args.model_type == "bert":
						_, contrast_sent_outputs = model(batch["contrast_input_ids"][:,i,:],
														 attention_mask=batch["contrast_attention_mask"][:,i,:],
														 token_type_ids=batch["contrast_token_type_ids"][:,i,:],
														 do_contrast=True)
					else:
						_, contrast_sent_outputs = model(batch["contrast_input_ids"][:,i,:],
														 attention_mask=batch["contrast_attention_mask"][:,i,:],
				...
				all_contrast_outputs = torch.stack(all_contrast_sent_outputs, 1) / args.contrast_temp
				if args.contrast_lower_is_better:
					all_contrast_outputs = - all_contrast_outputs
				all_contrast_sentlabs = torch.zeros(len(all_contrast_outputs)).long().cuda()

				contrast_loss_fct = nn.CrossEntropyLoss()
				contrast_loss = contrast_loss_fct(all_contrast_outputs, all_contrast_sentlabs)

				loss = sent_loss * 5 + contrast_loss
...
def infer(args, model, tokenizer, infer_dataloader):
	...
	for batch in infer_dataloader:
		...
		with torch.no_grad():
			...
			word_preds = None
			sent_preds = None
			if args.do_contrast and args.only_contrast:
				if args.model_type == "bert":
					_, sent_outputs = model(batch["input_ids"],
											attention_mask=batch["attention_mask"],
											token_type_ids=batch["token_type_ids"],
											do_contrast=True)
				else:
...
if __name__ == '__main__':
	...
	parser.add_argument("--best_metric", type=str, default=None, choices=('pearson', 'spearmanr', 'mcc', 'f1_multi'),
		help='Best metric to save your model.')

	parser.add_argument("--has_additional_feature", action="store_true")
	parser.add_argument("--do_contrast", action="store_true")
